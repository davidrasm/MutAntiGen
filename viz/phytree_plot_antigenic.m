...
function handles = phytree_plot_antigenic(tr,varargin)
...
colorMatrix = cmapTypes; %colormap(cool(maxState)); %jet also works nice
    ...
    case {'left','right'}
        ...
        for lin = 1:(lineages-1)
            ...
            for seg = 1: numSegs
                ...
                h.BranchLines(lineCounter) = plot(lineageSegs.x{lin}(1:2,seg), Y(1:2,lin), 'color', colorMatrix(clustMap(lineageSegs.segStates{lin}(seg)),:), 'LineWidth', 2.0); lineCounter = lineCounter + 1;
            end
            % plot vertical line segments
            h.BranchLines(lineCounter) = plot(X(2:3,lin), Y(2:3,lin), 'color', colorMatrix(clustMap(lineageSegs.segStates{lin}(numSegs)),:), 'LineWidth', 2.0); lineCounter = lineCounter + 1;
            ...
            case {'left','right'}
                ...
                %h.LeafDots = plot(tr.x(leafIndex(tr.activeNodes(leafIndex))),...
                 ...
                 for i = 1:tr.numLeaves
                    ...
                    %Paint by state
                    leafState = clustMap(lineageSegs.segStates{i}(1));
