function [tr] = phytree_read(filename)
%PHYTREE_READ reads a SimMap formated tree from file.
%
...
%Convert SimMap tree to regular Newick while parsing lineage state
%annotation
simMapString = strin;
annotationPositions = regexp(strin, '[{}]');
annotationBlocks = regexp(strin, '{', 'split');
lineages = (length(annotationPositions) / 2) + 1; %includes the the root as a lineage
